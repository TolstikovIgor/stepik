...
SELECT 5, book_id, 2
FROM book, author
WHERE book.title = 'Лирика' AND author.name_author = 'Пастернак Б.Л.';

INSERT INTO buy_book (buy_id, book_id, amount)
SELECT 5, book_id, 1
FROM book, author
WHERE book.title = 'Белая гвардия' AND author.name_author = 'Булгаков М.А.';
...
/*Количество тех книг на складе, которые были включены в заказ с номером 5, уменьшить на то количество, которое
в заказе с номером 5  указано.*/
...
количество заказанных книг и  стоимость. Последний столбец назвать Стоимость. Информацию в таблицу занести в отсортированном
по названиям книг виде.*/
...
/*Завершить этап «Оплата» для заказа с номером 5, вставив в столбец date_step_end дату 13.04.2020, и начать 
следующий этап («Упаковка»), задав в столбце date_step_beg для этого этапа ту же дату.

Реализовать два запроса для завершения этапа и начала следующего. Они должны быть записаны в общем виде, чтобы его
можно было применять для любых этапов, изменив только текущий этап. Для примера пусть это будет этап «Оплата».*/
UPDATE buy_step, step
...
WHERE buy_step.buy_id = 5 AND buy_step.step_id = 2;
...
/*Придумайте один или несколько запросов корректировки данных
В таблицу genre добавить новый жанр - "Футуризм". Везде, где автор "Пастернак" поменять жанр на "Футуризм".*/
...
SET
    book.genre_id = (SELECT genre_id FROM genre WHERE name_genre = 'Футуризм')
WHERE name_author = 'Пастернак Б.Л.';

SELECT author.name_author, book.title, genre.name_genre
FROM genre
    JOIN book USING(genre_id)
    JOIN author USING(author_id)
GROUP BY name_author, title, name_genre;
+------------------+-----------------------+------------+
| name_author      | title                 | name_genre |
+------------------+-----------------------+------------+
| Булгаков М.А.    | Мастер и Маргарита    | Роман      |
| Булгаков М.А.    | Белая гвардия         | Роман      |
| Достоевский Ф.М. | Идиот                 | Роман      |
| Достоевский Ф.М. | Братья Карамазовы     | Роман      |
| Достоевский Ф.М. | Игрок                 | Роман      |
| Есенин С.А.      | Стихотворения и поэмы | Поэзия     |
| Есенин С.А.      | Черный человек        | Поэзия     |
| Пастернак Б.Л.   | Лирика                | Футуризм   |
+------------------+-----------------------+------------+
